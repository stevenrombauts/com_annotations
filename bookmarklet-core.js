...
Annotations.Assistent = new Class({ 
	...
	create: function(event)
	{
		...
		if(!this.currentTarget.annotated)
		{
			...
			var annotation = new Annotations.Annotation(this.currentTarget, {ordering: order});
	...
	save: function()
	{
		...
		if(this.request == null)
		{
			...
			this.request = new Request.JSON({
				url: 'index.php?option=com_annotations&format=json&view=annotation&task=savejson&cachekiller='+dummy,
		...
		this.request.cancel().send({
			data: {'annotations': JSON.encode(objects)}
		});
	},
	...
	updateTableKeys: function(json)
	{
		...
		for(var i=0;i<json.count;i++)
		{
			...
			for(var j=0;j<annotations.length;j++)
			{
				...
				{
					annotation.setTableKey(obj.table_key);
					continue;
				}
			}
		}
...
Annotations.Annotation = new Class({
	...
	initialize: function(element, options) 
	{
		...
		// save this instance into the annotation & source elements' storage
	...
	{ 
		...
		/*var property = 'left';
		var original = this.annotation.getPosition().x;
		var val = original - 20;
		
		// if current position doesn't equal 'left', set the correct values
		switch(this.options.position)
		{
			case 'right':
				val = original + 20;
			...
			case 'top':
				property = 'top';
				original = this.annotation.getPosition().y;
				val = original - 20;
				break;
			case 'top':
				property = 'top';
		...
		this.annotation.set('tween', {
			duration: 500,
			transition: Fx.Transitions.Sine.easeOut,
			link: 'chain'
		});
		
		this.annotation.tween(property, val).tween(property, original);*/
